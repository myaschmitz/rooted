...
export default function EditCareEventScreen() {
  ...
  const handlePickFromPlantPhotos = () => {
    const availablePhotos = plantPhotos.filter(photo => 
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedPhotos.length > 0) {
        try {
          await PhotoService.saveMultipleEventPhotos(event.plant_id, id, selectedPhotos);
        } catch (photoError) {
